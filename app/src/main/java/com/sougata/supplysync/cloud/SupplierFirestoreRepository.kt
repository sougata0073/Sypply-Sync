...
class SupplierFirestoreRepository {
    ...
    fun getSupplierPaymentsList(
        ...
        this.firestoreCommonRepository.getAnyModelsList(
            ...
            lastDocumentSnapshot = lastDocumentSnapshot,
            customSorting = FieldNames.SupplierPaymentsCol.TIMESTAMP to Query.Direction.ASCENDING,
            limit = limit,
            howToConvert = howToConvert,
            onComplete = onComplete
        )
        ...
        valuesCol.document(FieldNames.ValuesCol.SuppliersCountDoc.SELF_NAME)
            .get().addOnCompleteListener {
                if (it.isSuccessful) {
                    onComplete(
                        Status.SUCCESS,
                        Converters.numberToInt(it.result[FieldNames.ValuesCol.SuppliersCountDoc.VALUE] as Number),
    ...
    fun getDueAmountToSuppliers(onComplete: (Int, Double, String) -> Unit) {
        ...
        valuesCol.document(FieldNames.ValuesCol.SuppliersDueAmountDoc.SELF_NAME).get()
            .addOnCompleteListener {
                if (it.isSuccessful) {
                    onComplete(
                        Status.SUCCESS,
                        Converters.numberToDouble(it.result[FieldNames.ValuesCol.SuppliersDueAmountDoc.VALUE] as Number),
                        KeysAndMessages.TASK_COMPLETED_SUCCESSFULLY
                    )
                } else {
    ...
    fun getOrdersToReceive(onComplete: (Int, Int, String) -> Unit) {
        ...
        valuesCol.document(FieldNames.ValuesCol.OrdersToReceiveDoc.SELF_NAME).get()
            .addOnCompleteListener {
                if (it.isSuccessful) {
                    onComplete(
                        Status.SUCCESS,
                        Converters.numberToInt(it.result[FieldNames.ValuesCol.OrdersToReceiveDoc.VALUE] as Number),
                        KeysAndMessages.TASK_COMPLETED_SUCCESSFULLY
                    )
                } else {
                    onComplete(Status.FAILED, 0, it.exception?.message.toString())
    ...
    fun deleteSupplier(supplier: Supplier, onComplete: (Int, String) -> Unit) {
        ...
        this.usersCol.firestore.runTransaction {
            ...
            it.update(
                valuesCol.document(FieldNames.ValuesCol.SuppliersCountDoc.SELF_NAME),
                mapOf(FieldNames.ValuesCol.SuppliersCountDoc.VALUE to FieldValue.increment(-1))
            )
            it.update(
                valuesCol.document(FieldNames.ValuesCol.SuppliersDueAmountDoc.SELF_NAME),
                mapOf(FieldNames.ValuesCol.SuppliersDueAmountDoc.VALUE to FieldValue.increment(-supplier.dueAmount))
            )
        }.addOnCompleteListener {
    ...
    fun deleteSupplierItem(supplierItem: SupplierItem, onComplete: (Int, String) -> Unit) {
        ...
        this.usersCol.firestore.runTransaction {
            ...
            it.update(
                valuesCol.document(FieldNames.ValuesCol.SupplierItemsCountDoc.SELF_NAME),
                mapOf(FieldNames.ValuesCol.SupplierItemsCountDoc.VALUE to FieldValue.increment(-1))
            )
        }.addOnCompleteListener {
    ...
    fun deleteOrderedItem(orderedItem: OrderedItem, onComplete: (Int, String) -> Unit) {
        ...
        this.usersCol.firestore.runTransaction {
            ...
            if (!orderedItem.isReceived) {
                it.update(
                    valuesCol.document(FieldNames.ValuesCol.OrdersToReceiveDoc.SELF_NAME),
                    mapOf(FieldNames.ValuesCol.OrdersToReceiveDoc.VALUE to FieldValue.increment(-1))
                )
            }
        }.addOnCompleteListener {
            if (it.isSuccessful) {
    ...
    fun getPurchaseAmountByRange(
        ...
        val query = orderedItemsCol
            .whereGreaterThanOrEqualTo(FieldNames.OrderedItemsCol.ORDER_TIMESTAMP, startTimestamp)
            .whereLessThanOrEqualTo(FieldNames.OrderedItemsCol.ORDER_TIMESTAMP, endTimestamp)
            ...
            if (it.isSuccessful) {
                ...
                coroutineScope.launch(Dispatchers.IO) {
                    ...
                    for (doc in it.result.documents) {
                        val data = doc.data
                        if (doc.exists() && data != null) {
                            val itemName = data[FieldNames.OrderedItemsCol.ITEM_NAME] as String
    ...
    fun getSupplierPaymentsByRange(
        ...
        val query = supplierPaymentsCol
            ...
            .whereLessThanOrEqualTo(FieldNames.SupplierPaymentsCol.PAYMENT_TIMESTAMP, endTimestamp)
            .orderBy(FieldNames.SupplierPaymentsCol.PAYMENT_TIMESTAMP, Query.Direction.ASCENDING)
